...
class Incidents_Api_Object extends Api_Object_Core {
	...
	public function _get_incidents($where = array())
	{
		...
		$xml->startElement('service_requests');
		...
		foreach ($items as $item)
		{
			...
			$xml->writeElement('service_request_id',$item->incident_id);
			
			
			// $xml->writeElement('title',$item->incident_title);

			$xml->writeElement('description',$item->incident_title . ' - ' . $item->incident_description);
			$xml->writeElement('requested_datetime',$item->incident_date);
			...
			//$xml->writeElement('address_id',$item->location_id);
			$xml->writeElement('address',$item->location_name);
			$xml->writeElement('lat',$item->latitude);
			$xml->writeElement('long',$item->longitude);
				...
				if (isset($media_items[$item->incident_id]) AND count($media_items[$item->incident_id]) > 0)
				{
					...
					foreach ($media_items[$item->incident_id] as $media_item)
					{
						...
						else 
						{
							...
							{
								//$xml->writeElement('media_url', $upload_path.$media_item['medialink']);
							}
						
							if( $media_item['mediathumb'] != "" ) 
							{
								//$xml->writeElement('media_url', $upload_path.$media_item['mediathumb']);
							}
							...
							if( $media_item['mediathumb'] != "" AND $media_item['mediatype'] == 1 )
							{
								...
								$xml->writeElement('media_url', $url_prefix.$upload_path.$media_item['medialink']);
							}
							//$xml->endElement();
						}
					}
			...
			//$xml->writeElement('media_url','');

			// report service code from categories
			...
			if (isset($category_items[$item->incident_id]))
			{
				foreach ($category_items[$item->incident_id] as $category_item)
				{
					...
					else 
					{
						...
						$category_names = array( '006', '007', '008', '009' );
						$service_code = (string) $category_item['cid'];
						$service_code = (int) $service_code;
						$service_code = $category_names[$service_code];
						$xml->writeElement('service_code',$service_code);
						break;
					}
				}
			}
		...
		else 
		{
			...
			$xml->endElement(); //end service_requests
